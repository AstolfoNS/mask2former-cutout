  ...
  你的数据量是 7755 张，类别只有 2 类，且图像已统一到 512x512，这对 Swin-Small + Mask2Former 来说属于“中等数据量、低类别复杂度、高边界质量要求”的场
  ...
  关于“类别灾难性遗忘”：
  如果你直接 num_labels=2 + ignore_mismatched_sizes=True，person/car 的 COCO 分类头语义不会被完整继承，但视觉特征和 mask 生成能力会继承。对于 7755 张
  ...
  - 数据量：7755 张；
  - micro-batch steps / epoch：约 ceil(7755 / 8) = 970；
  - accumulation=2 后 optimizer steps / epoch：约 485。
